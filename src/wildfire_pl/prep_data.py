...
def size_shape_filter(crowns: gpd.GeoDataFrame, preds_df, resx, resy,
                      area_m2=(20,600), aspect=(0.4,2.5)):
...
def ndvi_gate(crowns: gpd.GeoDataFrame, tif_path, ndvi_min=0.25):
    ...
    for g in crowns.geometry:
        mask = geometry_mask([mapping(g)], transform=transform, out_shape=ndvi.shape, invert=True)
        arr = ndvi[mask]
