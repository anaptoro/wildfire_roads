...
def _tiny_rgba_raster(width=20, height=20, res=1.0):
    ...
    data[3] = 200  # NIR
    mem = MemoryFile()
    with mem.open(driver="GTiff", height=height, width=width, count=4,
                  dtype="uint8", transform=transform, crs="EPSG:3857") as dst:
        dst.write(data)
    return mem
...
def test_size_shape_filter():
    ...
    kept = size_shape_filter(gdf, preds_df, resx=1.0, resy=1.0,
                             area_m2=(10, 1000), aspect=(0.4, 2.5))
