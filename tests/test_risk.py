...
def _write_rgbn(path: Path, w=40, h=40, crs="EPSG:3857"):
    ...
    with rasterio.open(path, "w", driver="GTiff", height=h, width=w, count=4,
                       dtype="uint8", transform=transform, crs=crs) as dst:
        dst.write(arr)
...
def test_make_risk_and_spans(tmp_path):
    ...
    # one straight road 
    roads = gpd.GeoDataFrame(geometry=[LineString([(0,20),(40,20)])], crs="EPSG:3857")
    roads_gpkg = tmp_path/"roads.gpkg"
    roads.to_file(roads_gpkg, layer="lines", driver="GPKG")
    roads.assign(geometry=roads.buffer(5)).to_file(roads_gpkg, layer="lines_buffer", driver="GPKG")

    # two crown boxes near the line
    crowns = gpd.GeoDataFrame(geometry=[box(10,15,12,17), box(28,22,30,24)], crs="EPSG:3857")
